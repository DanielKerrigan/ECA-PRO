...
This code is from https://github.com/adobe/pardon/tree/main/packages/favor
...
interface ForgedConfigEnv<K extends keyof VitePluginConfig = keyof VitePluginConfig>
	extends ConfigEnv {
	root: string;
...
export function extendMainConfig(config: UserConfigExport): UserConfigExport {
	return async (env: ConfigEnv) =>
		await applyBaseBuildConfig(env as ForgedConfigEnv<'build'>, config, 'main', 'es');
}

export function extendPreloadConfig(config: UserConfigExport): UserConfigExport {
	return async (env: ConfigEnv) =>
		await applyBaseBuildConfig(env as ForgedConfigEnv<'build'>, config, 'preload', 'cjs');
}

export function extendRendererConfig(config: UserConfigExport): UserConfigExport {
	return async (env: ConfigEnv) =>
		await applyBaseRendererConfig(env as ForgedConfigEnv<'renderer'>, config);
}

const builtins = ['electron', ...builtinModules, ...builtinModules.map((m) => `node:${m}`)];

const external = [...builtins, ...Object.keys(pkg['dependencies'] ?? {})];

// not quite sure why we need to smuggle this value via a global object.
const viteDevServers = ((process as any).viteDevServers ??= {}) as Record<string, ViteDevServer>;

function isPromiseForUserConfig(config: UserConfigExport): config is Promise<UserConfig> {
	...
	const define = env.forgeConfig.renderer
		...
		.reduce<Record<string, string>>((define, name) => {
			...
			define[devServerUrlKey] = JSON.stringify(
				command === 'serve'
					? `http://localhost:${
							(viteDevServers[name!].httpServer?.address?.() as AddressInfo)?.port
						}`
					: undefined
			);
...
async function applyBaseRendererConfig(env: ForgedConfigEnv<'renderer'>, config: UserConfigExport) {
	const {
		root,
		mode,
